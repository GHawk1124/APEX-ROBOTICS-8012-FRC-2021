...
{
  auto leftSideEncoders = (m_LF.GetSelectedSensorPosition() + m_LB.GetSelectedSensorPosition()) / 2;
  auto rightSideEncoders = (m_RF.GetSelectedSensorPosition() + m_RB.GetSelectedSensorPosition()) / 2;
    m_odometry.Update(m_gyro.GetRotation2d(),
                    units::meter_t(leftSideEncoders),
...
frc::DifferentialDriveWheelSpeeds DriveSubsystem::GetWheelSpeeds() {
  auto leftRate = (m_LF.GetSelectedSensorVelocity()+m_LB.GetSelectedSensorVelocity())/2;
  auto rightRate = (m_RF.GetSelectedSensorVelocity()+m_RB.GetSelectedSensorVelocity())/2;
